...
# out_dir = 'D:/User/Sisi_Tao/rotated_downsampled_data_4channels'
in_dir = 'C:/Users/Stadtpilot/Desktop/datasets/example_2'
out_dir = 'C:/Users/Stadtpilot/Desktop/datasets/example_3'
...
for lidar_file in lidar_list:
    ...
    lidar_data = np.reshape(lidar_data, [3, 64, 512])
    distance = lidar_data[0]
    lidar_data_new = np.zeros([4, 64, 512],dtype=np.float32)
    lidar_data_new[0:3] = lidar_data
    ...
    for i in range(64):
        for j in range(512):
            ...
            b = 1 + np.square(np.tan(theta))
            lidar_data_new[3][i][j] = np.float32(d / np.sqrt(a * b))
            if j>128 and j<384:
                lidar_data_new[3][i][j]=0-lidar_data_new[3][i][j]
    lidar_data_new = np.reshape(lidar_data_new, [131072])
