    ...
    with tf.Graph().as_default():
        ...
        for _ in range(nrof_preprocess_threads):
            ...
            for filename in tf.unstack(filenames):
                ...
                lidarscan = tf.reshape(lidarscan, [nrof_channel, 400*400])
                ...
                if nrof_channel == 2:
                    height = lidarscan[0]
                    reflection = lidarscan[1]
                    lidarscan = tf.reshape(tf.stack([height, reflection], axis=1), (400, 400, nrof_channel))
                if nrof_channel == 3:
                    height = lidarscan[0]
                    reflection = lidarscan[1]
                    density = lidarscan[2]
                    lidarscan = tf.reshape(tf.stack([height, reflection, density], axis=1),
                                           (400, 400, nrof_channel))
        ...
        lidarscan_batch, labels_batch, dist_batch = tf.train.batch_join(
            ...
            shapes=[(400, 400, nrof_channel), (), ()], enqueue_many=True,
...
def parse_arguments(argv):
    ...
    parser.add_argument('--logs_base_dir', type=str,
                        ...
                        default='D:/User/Sisi_Tao/Masterarbeit/logs/train_distance_bev')
    parser.add_argument('--models_base_dir', type=str,
                        help='Directory where to write trained models and checkpoints.',
                        default='D:/User/Sisi_Tao/Masterarbeit/models/train_distance_bev')
    parser.add_argument('--gpu_memory_fraction', type=float,
                        help='Upper bound on the amount of GPU memory that will be used by the process.', default=1.0)
    parser.add_argument('--pretrained_model', type=str,
                        help='Load a pretrained model before training starts.')
    # default='../../models/train_distance_bev/20190131-120604/model-20190131-120604.ckpt-2840'
    parser.add_argument('--data_dir', type=str,
                        help='Path to the data directory.',
                        default='D:/User/Sisi_Tao/bev')
    ...
    parser.add_argument('--pairs_file', type=str,
                        ...
                        default='D:/User/Sisi_Tao/pairs_file_40_bev.txt')
    parser.add_argument('--map_pairs_file', type=str,
                        help='Path to the pairs_file.',
                        default='D:/User/Sisi_Tao/pairs_based_on_map_15_bev.txt')
    parser.add_argument('--model_def', type=str,
                        help='Model definition. Points to a module containing the definition of the inference graph.',
                        default='model_SqueezeNet_bev')
    parser.add_argument('--nrof_validation_pairs', type=int,
                        help='Number of pairs for validation', default=256)
    parser.add_argument('--max_nrof_epochs', type=int,
                        help='Number of epochs to run.', default=1000)
    parser.add_argument('--pairs_per_batch', type=int,
                        help='Number of pairs per batch.', default=16)
    ...
    parser.add_argument('--learning_rate_decay_epochs', type=int,
                        help='Number of epochs between learning rate decay.(not used)', default=50)
