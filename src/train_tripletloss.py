...
def main(args):
    ...
    train_set = my_net.get_dataset(args.data_dir)
    validation_set = train_set  # 临时的！！！！！！！！！！！！

    # reform validation_set to pairs with issame labeled
    validation_paths = []
    actual_issame = []
    # pairs contain lidarscans from the same class/place
    ...
    # pairs contain lidarscans from the different classes/places
    for j in range(2):
        idx_pairs = []
        for i in range(len(validation_set)):
            class1_idx = class2_idx = 0
            while class1_idx == class2_idx or class1_idx == i or class2_idx == i:
                class1_idx, class2_idx = np.random.randint(len(validation_set), size=2)
                if (class1_idx, i) in idx_pairs:
                    class1_idx = np.random.randint(len(validation_set))
                if (class2_idx, i) in idx_pairs:
                    class2_idx = np.random.randint(len(validation_set))
            validation_paths += (validation_set[i].lidarscan_paths[j], validation_set[class1_idx].lidarscan_paths[j])
            validation_paths += (validation_set[i].lidarscan_paths[j], validation_set[class2_idx].lidarscan_paths[j])
            idx_pairs += ((i, class1_idx), (i, class2_idx))
    ...
    with tf.Graph().as_default():
        ...
        for _ in range(nrof_preprocess_threads):
            ...
            for filename in tf.unstack(filenames):
                ...
                coordinate = np.fromfile(coordinate_filename, dtype=np.float64)[16:18]

                # per lidarscan normalization
                mean = np.reshape(lidarscan.mean(axis=1), (2, 1))
                lidarscan -= mean
                std = np.reshape(lidarscan.std(axis=1), (2, 1))
                lidarscan /= std
            ...
            lidarscans_and_labels_and_coordinates.append(
                [lidarscans, label, coordinates])  # 循环结束得到四个地点的scans，每个地点多个scans

        lidarscan_batch, labels_batch, coordinates_batch = tf.train.batch_join(
            lidarscans_and_labels_and_coordinates, batch_size=batch_size_placeholder,
        ...
        prelogits, _ = network.inference(lidarscan_batch, args.keep_probability,
                                         is_training=is_training_placeholder,
                                         bottleneck_layer_size=args.embedding_size,
                                         weight_decay=args.weight_decay)
        # 有待设置weight regularization 11.18已经添加了reg_term
        ...
        learning_rate = learning_rate_placehloder  # 源代码用了exponential_decay
        ...
        with sess.as_default():
            ...
            while epoch < args.max_nrof_epochs:
                ...
                train(args, args.learning_rate_schedule_file, epoch, train_set, sess, enqueue_op,
                      ...
                      summary_writer)
                # 源代码里的summary_op并没有被sess运行。不确定summary有用！！！！明天试一下。但是summary_writer里有写sess
                # Save variables and the metagraph if it doesn't exist already
                save_variables_and_metagraph(sess, saver, summary_writer, model_dir, subdir, step)

                # Evaluate 源代码用了另一个数据集，这里前面应该把trainset划分一下
...
def train(args, learning_rate_schedule_file, epoch, dataset, sess, enqueue_op, lidarscan_paths_placeholder,
    ...
    while batch_number < args.epoch_size:
        ...
        triplets, nrof_random_negs, nrof_triplets = select_triplets(coordinates_array, num_per_class,
                                                                    lidarscan_paths, args.places_per_batch, args.alpha)
        ...
        # loss_array = np.zeros((nrof_triplets,))  # 不是应该nrof_batches吗,而且这个array好像用不上
        summary = tf.Summary()
        step = 0
        for i in range(nrof_batches):
            start_time = time.time()
            ...
            err, _, step, emb, lab = sess.run([loss, train_op, global_step, embeddings, labels_batch],
                                              feed_dict=feed_dict)
            emb_array[lab, :] = emb  # 这里其实用不上，因为train_op里已经计算了
            # loss_array[i] = err
...
def evaluate(sess, validation_paths, embeddings, enqueue_op, batch_size_placeholder, lidarscan_paths_placeholder,
    ...
    for i in range(nrof_batches):
        ...
        emb_array[lab, :] = emb  # 省略了 label_check_array,没有分成几个小的batch
    print('%.3f' % (time.time() - start_time))
...
def sample_places(dataset, places_per_batch, lidarscans_per_place):
    ...
    while len(lidarscan_paths) < nrof_lidarscans:
        ...
        nrof_lidarscans_in_class = len(dataset[class_index])  # dataset是一个LidarscanClass的列表，这个class的长度是其包含的Lidarscans的数量
        lidarscan_indices = np.arange(nrof_lidarscans_in_class)
        np.random.shuffle(lidarscan_indices)
        idx = lidarscan_indices[0:lidarscans_per_place]
        lidarscan_paths_for_class = [dataset[class_index].lidarscan_paths[j] for j in idx]
        lidarscan_paths += lidarscan_paths_for_class
        num_per_class.append(lidarscans_per_place)  # 这里所有的类中取出的点云数量是一样的，假设的情况是lidarscans_per_place<所有class中点云的数量，但源代码不是
        i += 1
